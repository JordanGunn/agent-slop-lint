  ...
  src/
    ...
    slop/                   <- the Python package (import slop)
      ...
      linter/               Linter, Result, Slop, dispatch, format, rule registry
        ...
        overlap.py          Cross-rule priority analysis: rank targets by converging concern families
...
- **Output leads with cross-rule priority, not just per-rule detail.** A flat per-rule listing answers "what does each rule find" but hides the fix-first signal — *independent concerns converging on one target*. `slop/linter/overlap.py` ranks files (drilling into symbols) by distinct concern *family*, where a family is the rule's namespace (`complexity.*`/`lexical.*`/`inheritance.*` collapse to one each, since their metrics are correlated; standalone rules are their own family). This surfaces as a "Priority" block atop the human report and a `priority` array in the JSON; the per-rule detail follows unchanged. Prerequisite: findings' `file` paths are normalised to one canonical repo-relative form in `Linter.run` (`_relativize`) — rules emit a mix of absolute/relative, which otherwise splits one file into two keys and corrupts the overlap ranking.

...
def test_coupling_violation_above_threshold(tmp_path: Path):
    ...
    rc = RuleConfig(enabled=True, severity="error", params={"coupling_threshold": 8})
...
def test_inheritance_depth_violation(tmp_path: Path):
    ...
    rc = RuleConfig(enabled=True, severity="error", params={"inheritance_depth_threshold": 3})
...
def test_inheritance_children_violation(tmp_path: Path):
    ...
    rc = RuleConfig(enabled=True, severity="error", params={"inheritance_children_threshold": 10})

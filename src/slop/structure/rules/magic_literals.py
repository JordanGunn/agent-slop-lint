"""Magic literal density rule — view-native (v2).
  ...
  structural.magic_literals  — flag functions with too many distinct
...
relocated to ``structural.*`` in v2.0 because it captures a structural
signal (missing parameterization), not a Halstead-derived information
measure. The legacy name resolves via ``slop._compat``.
"""
    ...
    for c in structure.callables():
        ...
        if len(non_trivial) > threshold:
            ...
            findings.append((len(non_trivial), Slop(
                rule="structural.magic_literals",
                file=rel,
                line=c.line,

...
Substrate-aligned replacement for ``slop._structural.deps``'s extraction
half. Per-grammar tree-sitter queries (declared on ``Language.import_queries``)
...
they contribute zero edges. This is a deliberate parity break vs the
legacy ``deps_kernel``.
"""
...
def _run_query(ts_lang, query_str: str, root_node, content: bytes):
    ...
    API and the legacy ``lang.query()`` API. Mirrors the dispatch in
    ``slop._ast.query._query_file`` but operates on an already-parsed
    tree.
    """
    import tree_sitter

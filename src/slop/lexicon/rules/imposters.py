...
from slop._lexical.imposters import _profile_cluster, imposters_kernel
...
def run_imposters(
    ...
    """v2 imposters — clustering via Lexicon view, signal compute via
    legacy ``_profile_cluster`` (re-used as a pure helper during the
    migration window).

    The Lexicon view owns the hierarchical first-parameter clustering
    (``lexicon.first_param_clusters``). Body-Jaccard / receiver-call
    density / modal-token overlap and the profile classifier are
    reused from the legacy kernel as pure functions — those compute
    methods migrate onto Lexicon in a follow-up intent.
    """
    ...
    # Signal computation: reuse the legacy ``_profile_cluster`` helper.
    # ``Lexicon.bodies_for_cluster`` builds the (file, name) -> (body_node, content)
    # map the helper expects.
    for cluster in clusters:
        bodies = lexicon.bodies_for_cluster(cluster)

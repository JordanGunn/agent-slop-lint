...
def run_sprawl_v2(
    lexicon: Lexicon,  # noqa: ARG001 — view contract; current impl delegates to legacy kernel
    rule_config: RuleConfig,
    slop_config: SlopConfig,
) -> RuleResult:
    """v2 sprawl — thin shim during migration.

    The formal-concept-lattice analysis (Wille 1982, Caprile & Tonella
    2000) is heavy domain-specific work; the legacy ``sprawl_kernel``
    performs it. This v2 wrapper takes a ``Lexicon`` view per the v2
    contract but delegates to the legacy kernel using
    ``slop_config.root``. When the FCA computation lands as a Lexicon
    compute method in a follow-up intent, this wrapper rewires.

    Emits ``Slop`` instead of ``Slop``; output structurally
    identical to ``run_sprawl``.
    """
    min_alphabet: int = int(rule_config.params.get("min_alphabet", 3))
    min_concept_extent: int = int(rule_config.params.get("min_concept_extent", 2))
    min_concept_intent: int = int(rule_config.params.get("min_concept_intent", 2))

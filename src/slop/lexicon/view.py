...
class Lexicon:
    ...
    def first_param_clusters(
        ...
        *,
        ...
        Returns ``FirstParameterCluster`` instances. Hierarchical
        traversal: a callable appears in at most one cluster, claimed at
        the deepest scope where the cluster reaches ``min_cluster``.

        ``root`` is the codebase root used to derive relative paths for
        the path-parts decomposition that drives scope claiming; if None,
        the longest common prefix across observed callable paths is used.

        The clustering walk and multi-signal enrichment live in
        ``lexicon.clusters``; this method is the view's delegating entry.
        """
        from slop.lexicon.clusters import compute_first_param_clusters

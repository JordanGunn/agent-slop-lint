...
- **Package carving — resolved.** Carving now uses `grammar.resolve_packages` (flat, `__init__`-gated, rel-path-named, matching the legacy package model), with a per-directory fallback for loose scripts. Verified: package boundaries + Ca/Ce/I/A match the oracle exactly on `src/slop` (15 packages). Residual: for **class-less packages** the legacy reports `A=None`/`distance=None`/zone `unknown`, while `PackageMetrics`'s non-optional float fields show `A=0.0` and a computed `distance`. Zone verdicts (`in_zone_of_pain`/`uselessness`) guard on definedness and are correct. Closing this exactly needs `PackageMetrics.{instability,abstractness,distance}: float | None` — a `component/metrics.py` change (the protected interfaces), deferred pending approval.

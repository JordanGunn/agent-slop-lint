...
from slop.fs.find import find_kernel
from slop.ast.treesitter import detect_language, load_language
...
def _cpp_find_function_identifier(node):
    """Walk a C++ ``function_definition``'s declarator chain. See
    ``slop._structural.ccx`` for the canonical implementation."""
    declarator = node.child_by_field_name("declarator")
    for _ in range(8):
        if declarator is None:
            return None
...
_LANG_CONFIG: dict[str, _HalsteadLangConfig] = {
    ...
    "c": _HalsteadLangConfig(
        function_nodes=frozenset({"function_definition"}),
        operator_types=frozenset({
            # Keywords (tree-sitter-c emits each as a named node whose
            # type IS the keyword string)

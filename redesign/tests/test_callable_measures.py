...
def test_magic_literals_excludes_trivials(tmp_path: Path):
    ...
    assert sorted(m.value for m in cfg.magic_literals()) == ["3600", "42"]  # 1 is trivial


def test_sentinel_parameters(tmp_path: Path):
    cfg = _cs(tmp_path)["x.cfg"]
    assert [s.name for s in cfg.sentinel_parameters()] == ["mode"]


def test_mutated_parameters(tmp_path: Path):
    mut = _cs(tmp_path)["x.mutate"]
    assert [(p.parameter, p.kind) for p in mut.mutated_parameters()] == [("items", "append")]

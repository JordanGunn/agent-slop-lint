...
def test_cyclomatic(tmp_path: Path):
    ...
    assert cs["m.h"].cyclomatic() == 6
    assert cs["m.branchy"].cyclomatic() == 3


def test_cognitive(tmp_path: Path):
    cs = _callables(tmp_path)
    assert cs["m.h"].cognitive() == 7
    assert cs["m.branchy"].cognitive() == 2


def test_combinatorial_npath(tmp_path: Path):
    cs = _callables(tmp_path)
    assert cs["m.h"].combinatorial() == 9
    assert cs["m.branchy"].combinatorial() == 3


def test_halstead(tmp_path: Path):
    cs = _callables(tmp_path)
    assert round(cs["m.h"].volume(), 2) == 76.15
    assert round(cs["m.h"].halstead_density(), 2) == 6.67
    prof = cs["m.h"].halstead()
...
def test_aggregation_sums_volume(tmp_path: Path):
    ...
    total = sum(c.volume() for c in corpus._iter_callables())
    assert round(mod.volume(), 4) == round(total, 4)
